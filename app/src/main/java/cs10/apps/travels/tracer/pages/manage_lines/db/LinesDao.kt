package cs10.apps.travels.tracer.pages.manage_lines.db
...
import cs10.apps.travels.tracer.pages.manage_lines.model.BusDayInfo
import cs10.apps.travels.tracer.pages.manage_lines.model.BusDestinationInfo
import cs10.apps.travels.tracer.pages.manage_lines.model.BusRamalInfo
import cs10.apps.travels.tracer.pages.manage_lines.model.TrainDayInfo
...
interface LinesDao {
    ...
    @Query("SELECT L.*, AVG(V.rate) as avgUserRate, COUNT(V.rate) as reviewsCount " +
            ...
            "GROUP BY L.id HAVING COUNT(V.rate) > 0")
    ...
    fun getAllRecentWithRates(currentYear: Int, startMonth: Int) : MutableList<RatedBusLine>
